---
name: moyi-resource-sheet-parser
description: 解析心遇活动资源配置表格（POPO 在线表格），提取任务 sheet 和抽奖 sheet 数据为结构化 JSON。供 moyi-activity-create 主 skill 调用。
---

# moyi-resource-sheet-parser
...
- 本 skill 不负责权重和校验（100%），也不负责库存类型映射失败的处理，这些由下游 moyi-lottery-clone 处理
