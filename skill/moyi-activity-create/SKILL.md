...
```bash
mws moyi-activity-backend plan-list --env <env> --params '{"name": "<关键词>", "page": {"from": 0, "to": 1, "size": 20}}' --format json
```

**注意**：`page` 为必填参数（PageRequest JSON 字符串），必须包含 `from`、`to`、`size`。
...
```bash
mws moyi-activity-backend plan-create --env <env> --body '{"name": "<新plan名>", "startTime": "<毫秒>", "endTime": "<毫秒>", "modules": ["mission","spinach","wechat"], "domain": "<沿用源>", "noticeCorps": ["<沿用源>"]}' --format json
```

**注意**：plan-create 是 JSON body 接口（`application/json`），时间戳为**字符串**类型（如 `"1779778800000"`），不能用 `--params` 传参。
...
收到链接后调用 `moyi-resource-sheet-parser`：
...
#### 6.3 小程序活动模块 — moyi-act-resource-clone
...
调用 popo-group-cli skill 发送消息到指定群并 @创建者：

```bash
popo-group send --group 8401763 --at <创建者corp账号> --message "<汇总消息>"
```

**创建者识别**：取当前会话的 sender_id 作为创建者 corp 账号。
...
若消息超过 3000 字符，拆分为多条按顺序发送（群消息和私聊各发一组）。

**发送顺序**：先发私聊，再发群消息。若群消息发送失败，私聊中补充提示"群通知发送失败，请手动同步到群 8401763"。
...
| M2 | 源 mission box id → 新 box id | moyi-mission-clone |
| M3 | 源 tenantId → 新 tenantId | moyi-lottery-clone |
| M4 | 源 token → 新 token | moyi-lottery-clone |
| M5 | 源 interestId → 新 interestId | moyi-lottery-clone |
| M6 | 源 type=7 act-resource id → 新 id | moyi-act-resource-clone |
...
| 配置项 | 值 | 说明 |
|--------|-----|------|
| 通知群号 | `8401763` | 完成汇总发送到此群，@创建者 |
| 创建者识别 | 当前会话 sender_id | 作为 @at 目标和私聊接收方 |
| 发送顺序 | 先私聊 → 再群@ | 群发送失败时私聊补充提示 |
| 群发送工具 | `popo-group send` | 唯一支持 POPO @提醒的方式 |
