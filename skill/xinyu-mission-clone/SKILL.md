---
name: xinyu-mission-clone
description: 心遇任务模块克隆子 skill。接收源/新 mission panel activityId + 任务资源数据，调用 copy-mundo 克隆 mundo 树，反查替换 rewardBoxId，构建 M2 映射表。供 xinyu-plan-clone 主 skill 调用。
---
...
- `missionEntries`: 任务资源数据（来自 xinyu-resource-sheet-parser）
- `env`: 环境（online / test）
